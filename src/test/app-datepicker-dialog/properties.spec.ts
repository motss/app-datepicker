  ...
  describe('updates via properties', () => {
    ...
    it(`renders with correct 'min'`, async () => {
      ...
      const firstSelectableDate =
        getSelectableDateEl<AppDatepickerDialog, HTMLTableDataCellElement>(el, 'Jan 15, 2020');
      const allDisabledDates =
        getAllDisabledDatesEl<AppDatepickerDialog, HTMLTableDataCellElement>(el);
      const focusedDate = getFocusedDateEl<AppDatepickerDialog, HTMLTableDataCellElement>(el);
      const lastDayBeforeMinDate = allDisabledDates.reduce((
        p: HTMLTableDataCellElement,
        n: HTMLTableDataCellElement
      ) => {
        const pDay = +p.day;
        const nDay = +n.day;
      ...
      const newFocusedDateWithoutMin = getFocusedDateEl(el);
