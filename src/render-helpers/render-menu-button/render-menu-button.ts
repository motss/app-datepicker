...
export function renderMenuButton({
  ...
  text,
  type,
}: RenderMenuButtonInit): TemplateResult {
  ...
  return html`
  <md-text-button class=${classMap({ menuButton: true, ...clsObject })} data-type=${ifDefined(type)} trailing-icon @click=${onClick} aria-label=${label} title=${label}>
    ${text}
