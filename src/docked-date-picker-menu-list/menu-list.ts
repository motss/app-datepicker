...
export class MenuList extends DatePickerMinMaxMixin(RootElement) implements MenuListProperties {
  ...
  #updateFormatter = (changedProperties: PropertyValueMap<this>) => {
    if (changedProperties.has('locale') && changedProperties.get('locale') !== this.locale) {
      this.#monthFormat = new Intl.DateTimeFormat(this.locale, longMonthFormatOptions).format;
      this.#yearFormat = new Intl.DateTimeFormat(this.locale, yearFormatOptions).format;
      this.requestUpdate();
    }
  };
  ...
  #updateMenuItemTemplates = (changedProperties: PropertyValueMap<this>) => {
    if (
      (changedProperties.has('menuListType') && changedProperties.get('menuListType') !== this.menuListType) ||
      (changedProperties.has('monthMenuItemTemplate') && changedProperties.get('monthMenuItemTemplate') !== this.monthMenuItemTemplate) ||
      (changedProperties.has('yearMenuItemTemplate') && changedProperties.get('yearMenuItemTemplate') !== this.yearMenuItemTemplate) ||
      (changedProperties.has('selectedMonthMenuItemTemplate') && changedProperties.get('selectedMonthMenuItemTemplate') !== this.selectedMonthMenuItemTemplate) ||
      (changedProperties.has('selectedYearMenuItemTemplate') && changedProperties.get('selectedYearMenuItemTemplate') !== this.selectedYearMenuItemTemplate)
    ) {
      const {
        menuListType,
      ...
      const template = isMonthMenu ? monthMenuItemTemplate : yearMenuItemTemplate;
      const selectedTemplate = isMonthMenu ? selectedMonthMenuItemTemplate : selectedYearMenuItemTemplate;

      this.#menuItemTemplateFn = (arg: number) => templateReplacer(template, [arg]);
      this.#selectedMenuItemTemplateFn = (arg: number) => templateReplacer(selectedTemplate, [arg]);
      this.requestUpdate();
    }
  };
  ...
  #updateMenuList = (changedProperties: PropertyValueMap<this>) => {
    if (
      (changedProperties.has('menuListType') && changedProperties.get('menuListType') !== this.menuListType) ||
      (changedProperties.has('min') && changedProperties.get('min') !== this.menuListType) ||
      (changedProperties.has('max') && changedProperties.get('max') !== this.menuListType) ||
      (changedProperties.has('value') && changedProperties.get('value') !== this.menuListType)
    ) {
      const {
        max,
        menuListType,
      ...
      switch (menuListType) {
        ...
        default: {
          ...
          list = Array.from<unknown, MenuListItem>(Array(maxYear - minYear + 1), (_, i) => {
  ...
  @property({ attribute: false }) onMenuChange?: MenuListProperties['onMenuChange'];
  @property() selectedMonthMenuItemTemplate: string = labelSelectedMonthMenuItemTemplate;
  @property() selectedYearMenuItemTemplate: string = labelSelectedYearMenuItemTemplate;
  ...
  protected override render(): TemplateResult {
    ...
    >${
      map(_menuList, ({
        ...
        const ariaLabel = (selected ? this.#selectedMenuItemTemplateFn : this.#menuItemTemplateFn)(value);
